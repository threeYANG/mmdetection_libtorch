...
#define CHECK_BINARY_OP(name, op, x, y)                                 \
  if (tunicornvideostruct::LogCheckError _check_err = tunicornvideostruct::LogCheck##name(x, y))    \
    tunicornvideostruct::LogMessageFatal(__FILENAME__, __LINE__).stream()                 \
...
#define CHECK_NOTNULL(x) \
  ((x) == NULL ? tunicornvideostruct::LogMessageFatal(__FILENAME__, __LINE__).stream() << "Check  notnull: "  #x << ' ', (x) : (x)) // NOLINT(*)
// Debug-only checking.
#ifdef NDEBUG
#define DCHECK(x) \
  while (false) CHECK(x)
...
#define LOG_INFO tunicornvideostruct::LogMessage(__FILENAME__, __LINE__)

#define LOG_ERROR LOG_INFO
#define LOG_WARNING LOG_INFO
#define LOG_FATAL tunicornvideostruct::LogMessageFatal(__FILENAME__, __LINE__)
...
#define LOG_IF(severity, condition) \
  !(condition) ? (void)0 : tunicornvideostruct::LogMessageVoidify() & LOG(severity)
